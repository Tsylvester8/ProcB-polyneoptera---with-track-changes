...
% TS: added a paragraph to state that the ChromPlus model that we used can incorporate binary traits and see the impact on these binary traits on chromosome number evolution. - reply for R1.4
\textcolor{blue}{When compared with similar models of chromosome number evolution (e.g. ChromEvol), the ChromPlus model that we use here to model chromosome number evolution can also be used to look at the impact of binary states (i.e. states that can be classified as 1 and 0) on chromosome number evolution \cite{blackmon2019meiotic}. We used this capability of the ChromPlus model to asses the impact on mode of reproduction on chromosome number evolution. In this analysis we used the second phylogeny we inferred for Phasmatodea and ran an MCMC es explained above. We used sexual reproduction and parthenogenesis as our binary states.}
...
When we have multiple records for genome size for a given species we used the mean value. Once we looked at species-level matches, to increase the overlap between the genome size dataset and the chromosome dataset, we looked at genera-level matches as well. For these chimeric taxa, genome size and chromosome number were randomly sampled.}
...
\textcolor{blue}{It must be noted that we do not find any evidence for aneuploidy in Phasmatodea. 
In fact, we do not find intraspecific aneuploidy reported for any of the clades used in this study.In finding aneuploid species/strains, individual level sampling and using molecular methods for increased accuracy in karyotyping (e.g. florescence in situ hybridisation) has been proposed elsewhere \cite{liehr2017new}.}
...
The power of the three linear models were reported as XX\%, XX\%, and XX\% respectivly for fusions, fissions and polyploidy. 
...
\subsection{\textcolor{blue}{How our results stack against previous predictions}}
\textcolor{blue}{We find that many of our results confirm previous hypotheses on chromosome number evolution and sex chromosome system evolution in Polyneoptera while some of our results contradicts previous hypotheses.
In many instances our inference on the ancestral state of the sex chromosome system matches with what is predicted for that order. For instance, in Mantodea, Orthoptera and Phasmatodea it has been hypothesized that the ancestral state of the sex chromosome system as being XO and our results confirm these hypotheses \cite{hughes1950chromosomes,blackmon2016, blackman1995sex}. 
...
However, while it has been hypothesized that the most recent ancestor of Blattodea (including Isoptera) as being XY and the Y chromosome being rapidly lost in Blattodea (excluding Isoptera), our results show that the most recent common ancestor of Blattodea (including Isoptera) was in fact XO (with a probability of XX\%) \cite{luykx1990cytogenetic,bergamaschi2007karyology}.
Also, in Isoptera, while it has been hypothesised that the ancestral sex chromosome system as being XO, despite the fact that primitive Isoptera having XY sex chromosome system, our results suggests that ancestral sex chromosome system of Isopteara was in fact XY (with a probability of XX\%) \cite{luykx1990cytogenetic,blackman1995sex}.}
 
\textcolor{blue}{Although we do not find many predictions about the processes that govern chromosome number evolution for majority of the orders in Polyneoptera, we do find some instances where such processes are hypothesiesd.
...
Although we do not find a significant difference between the rates of fusion and rates of fission (Table S3), in our ancestral state analysis for chromosome number, we find that on average number of fusion events are high than number of fission events (on average 33.32 fusion events and 21.94 fission events).} 

\textcolor{blue}{Unfortunately, our results on ancestral states of the chromosome number does not stack well against previous predictions.
For instance, in Mantodea, we inferred that 8 being the most probable chromosome number and 7 as being the second most probable chromosome number of the ancestor (with probabilities 41.2\% and 19.2\%). However, our predictions does not come close to the previously predicted ancestral chromosome number which is 14 \cite{hughes1950chromosomes}. 
In many instances, the most common number in a given clade is identified as its ancestral chromosome number.
However, most our inferences on the ancestral state of the chromosome number are further away from that of the predicted ancestral chromosome number for a given clade except for Isoptera (where the mode of the dataset was 21 while the ancestral chromosome number was estimated as 20 with a probaboloty of 35.6\%). These results on the inference of ancestral states for chromosome number suggests that we need much broader level sampling of the extant species to get accurate estimation of the ancestral states.}
...
% TS: added terms positive and negative to avoid any confusion. reply for R2.12
